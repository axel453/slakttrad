(function(){
  ...
  function bind(){
    ...
    document.addEventListener('input',event=>{if(event.target.matches('[data-gallery-caption]'))updateGalleryCaption(event.target);else if(event.target.matches('[data-gallery-source]'))refreshGalleryManager(event.target.closest('[data-gallery-manager]'));else if(event.target.matches('[data-profile-url]'))setProfilePreview(event.target.closest('[data-profile-manager]'),event.target.value.trim());});
