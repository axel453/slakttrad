(function(){
  ...
  function personRow([id,p]){ const hay=[p.name,p.alt,p.born,p.died,p.place,p.role].join(' ').toLocaleLowerCase('sv');return `<tr data-search="${esc(hay)}" data-branch="${esc(p.branch||'shared')}"><td><div class="entity-cell"><img class="entity-avatar" src="${esc(p.photo||'/assets/person-placeholder.svg')}" alt=""><span><strong>${esc(p.name)}</strong><small>${esc(p.role||'Person')}</small></span></div></td><td>${esc([p.born,p.died].filter(Boolean).join(' – ')||'Saknas')}</td><td>${esc(p.place||'Ej angivet')}</td><td><span class="badge">${esc(labels[p.branch]||'Gemensamt')}</span></td><td><span class="badge ${p.status==='confirmed'?'green':'amber'}">${esc(p.status||'open')}</span></td><td><button class="row-action" data-edit-person="${esc(id)}">Redigera</button></td></tr>`; }
  ...
  function placeRow(p){const mapped=Number.isFinite(Number(p.lat))&&Number.isFinite(Number(p.lng));const hay=[p.name,p.area,...(p.aliases||[])].join(' ').toLocaleLowerCase('sv');return `<tr data-search="${esc(hay)}" data-map="${mapped?'mapped':'unmapped'}"><td><div class="entity-cell"><span class="entity-avatar entity-icon">${icon('landmark')}</span><span><strong>${esc(p.name)}</strong><small>${esc(p.note||'Platskort')}</small></span></div></td><td>${esc(p.area||'Ej angivet')}</td><td>${esc((p.aliases||[]).slice(0,3).join(', ')||'Saknas')}</td><td><span class="badge ${mapped?'green':'amber'}">${mapped?'Kartlagd':'Saknas'}</span></td><td><button class="row-action" data-edit-place="${esc(p.id)}">Redigera</button></td></tr>`;}
  ...
  function renderPersonEditor(id){
    ...
    ui.content.innerHTML=`<div class="editor-page">${heading(title,isNew?'Skapa en ny person med ett stabilt internt ID.':'Uppdatera personens samlade arkivuppgifter.',`<button class="secondary-button" data-go="people">${icon('arrow-left')} Till registret</button>`)}<form id="personForm" class="editor-layout"><div class="editor-card">
      ...
      <section class="editor-section"><h2>Bilder</h2><p>Bildadress och valfri bildtext separeras med ett lodrätt streck.</p><div class="form-grid">${field('Profilbild','fPhoto',p.photo||'',true)}${field('Bildgalleri','fImages',imageText(p.images),true,'textarea')}</div></section></div>
  ...
  function renderPlaceEditor(id){
    ...
    ui.content.innerHTML=`<div class="editor-page">${heading(isNew?'Ny plats':p.name,isNew?'Skapa ett nytt gårds- eller platskort.':'Samla platsens historia och geografiska uppgifter.',`<button class="secondary-button" data-go="places">${icon('arrow-left')} Till registret</button>`)}<form id="placeForm" class="editor-layout"><div class="editor-card">
      ...
      <section class="editor-section"><h2>Bilder</h2><div class="form-grid">${field('Bildgalleri','fImages',imageText(p.images),true,'textarea','En per rad: bildadress | bildtext')}</div></section></div>
