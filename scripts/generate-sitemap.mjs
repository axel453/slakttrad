...
const code = fs.readFileSync(new URL("data.js", ROOT), "utf8") +
  "\nglobalThis.__siteData = { PEOPLE, PLACES, PARTNER, DIRECT_HEIRS };";
const context = {};
vm.runInNewContext(code, context, {filename:"data.js"});
const { PEOPLE, PLACES, PARTNER } = context.__siteData;
...
const paths = ["/","/personarkiv/","/gardar/",...Object.keys(PEOPLE).map(personUrl),...PLACES.map(placeUrl)];
