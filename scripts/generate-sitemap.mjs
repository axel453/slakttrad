...
function portraitHTML(person,className="detail-photo"){
  ...
  const x=clamp(crop.x,0,100,50),y=clamp(crop.y,0,100,50),zoom=clamp(crop.zoom,1,3,1);
  const photo=person?.photo||person?.image||"/assets/person-placeholder.svg";
  return `<span class="portrait-frame ${className}" style="--photo-x:${x}%;--photo-y:${y}%;--photo-zoom:${zoom};"><img class="portrait-image" src="${escapeHtml(photo)}" alt="Porträtt av ${escapeHtml(person?.name||"personen")}" loading="eager"></span>`;
}
